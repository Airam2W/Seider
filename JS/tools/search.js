...
function renderPills(containerId, values, selectedSet) {
    ...
    values.forEach((value) => {
        ...
        pill.className = "pill";
        pill.textContent = translateStoredLabel(value);
...
export async function startSearch() {
    ...
    const results = entries.filter((entry) => {
        ...
        const matchKeyword = searchKeywords.some((word) => note.includes(word));

        const matchItems = entry.items.some((item) => {
            return selectedTags.has(item.tag) || selectedSubtags.has(item.subtag);
        });

        return (
            matchKeyword ||
            matchItems ||
            (searchKeywords.length === 0 && selectedTags.size === 0 && selectedSubtags.size === 0)
        );
    });
...
async function runLiveSearch() {
    ...
    const results = entries.filter((entry) => {
        ...
        const entryTags = new Set(entry.items.map((item) => item.tag));
        const matchTags =
            selectedTags.size === 0 ||
            [...selectedTags].every((tag) => entryTags.has(tag));

        const entrySubtags = new Set(entry.items.map((item) => item.subtag));
        const matchSubtags =
            selectedSubtags.size === 0 ||
            [...selectedSubtags].every((subtag) => entrySubtags.has(subtag));
...
function renderSearchResults(entries) {
    ...
    if (entries.length === 0) {
        container.innerHTML = `<p style="opacity:0.6;">${t("timeline.noResultsFound")}</p>`;
        return;
    }
    ...
    entries.forEach((entry) => {
        ...
        div.innerHTML = `
            ...
            <div class="tags-preview">
                ${entry.items.map((item) => `${translateStoredLabel(item.tag)}/${translateStoredLabel(item.subtag)}`).join(", ")}
            </div>
            <div class="notes">${t("labels.notes")} ${decryptData(entry.notes) || "-"}</div>
            <br>
            <button onclick="viewEntryFromSearch('${entry.id}')">${t("buttons.viewEdit")}</button>
        `;
...
document.getElementById("searchResults").innerHTML = `<p style="opacity:0.6;">${t("timeline.noFiltersApplied")}</p>`;
